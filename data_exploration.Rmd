...
pnts_sf <- st_as_sf(pnts, coords = c("lon", "lat"), crs = st_crs(map))

pnts <- pnts_sf %>% 
  mutate(
    intersection = as.integer(st_intersects(geometry, map)), area = is.na(intersection), #if_else(is.na(intersection), '', map$SUA_NAME16[intersection])
  ) %>%
  mutate(
...
pnts %>%
  ggplot() +
  geom_point(aes(x = lon, y = lat, color = area), size = 1) +
  coord_fixed()
```
